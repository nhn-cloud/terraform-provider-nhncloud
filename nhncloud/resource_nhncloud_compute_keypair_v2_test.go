		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2KeypairExists("nhncloud_compute_keypair_v2.kp_1", &keypair),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestMatchResourceAttr(
						"nhncloud_compute_keypair_v2.kp_1", "fingerprint", fingerprintRe),
					resource.TestMatchResourceAttr(
						"nhncloud_compute_keypair_v2.kp_1", "private_key", privateKeyRe),
				),
			},
		},
	})
...
func testAccCheckComputeV2KeypairDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating NHN Cloud compute client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "nhncloud_compute_keypair_v2" {
			continue
		}
...
resource "nhncloud_compute_keypair_v2" "kp_1" {
  name = "kp_1"
