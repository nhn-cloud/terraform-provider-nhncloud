...
// VPCCreateOpts represents the attributes used when creating a new VPC.
type VPCCreateOpts struct {
	vpcs.CreateOpts
	//ValueSpecs map[string]string `json:"value_specs,omitempty"`
}
...
// ToNetworkCreateMap casts a CreateOpts struct to a map.
// It overrides networks.ToNetworkCreateMap to add the ValueSpecs field.
func (opts VPCCreateOpts) ToNetworkCreateMap() (map[string]interface{}, error) {
	return BuildRequest(opts, "vpc")
}
